...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        client.message_callback_add(
            MQTT_TOPIC["LED"].format(client._client_id), message_led
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog=APP_NAME, description="Reader RFID is client waresys"
    )
    parser.add_argument(
        "-v",
        "--version",
        ...
        try:
            ...
            if uid is not None:
                ...
                infot = client.publish(
                    MQTT_TOPIC["TAG"].format(client._client_id),
                    json.dumps({"uid": uid_hex}),
                )
